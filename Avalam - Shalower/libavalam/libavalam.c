...
void afficherPosition(T_Position p)
{
	...
	for (i = 0; i < NBCASES; i++)
	{
		...
		if (p.evolution.bonusJ == i)
			delta++;
		if (p.evolution.malusR == i)
			delta--;
		if (p.evolution.malusJ == i)
			delta--;
		if (delta > 0)
			printf(" [Evolution:+%d]\n", delta);
		else if (delta < 0)
...
T_ListeCoups getCoupsLegaux(T_Position p)
{
	...
	for (cO = 0; cO < NBCASES; cO++)
	{
		...
		for (iV = 0; iV < v.nb; iV++)
		{
			...
			if (p.cols[cD].nb == VIDE)
			{
				printf0("vide!\n");
				continue;
			} // ajout par tomnab 21/03/2018
...
T_Position jouerCoup(T_Position p, octet origine, octet destination)
{
	...
	if (!estValide(p, origine, destination))
		return p;
	...
	if (p.evolution.bonusR == origine)
		p.evolution.bonusR = destination;
	if (p.evolution.bonusJ == origine)
		p.evolution.bonusJ = destination;
	if (p.evolution.malusR == origine)
		p.evolution.malusR = destination;
	if (p.evolution.malusJ == origine)
		p.evolution.malusJ = destination;
...
T_Score evaluerScore(T_Position p)
{
	...
	// partie évolution
	if ((p.cols[p.evolution.bonusR].nb != VIDE) && (p.cols[p.evolution.bonusR].couleur == JAU))
	{
		s.nbJ++;
	}
	if ((p.cols[p.evolution.bonusR].nb != VIDE) && (p.cols[p.evolution.bonusR].couleur == ROU))
	{
		s.nbR++;
	}
	if ((p.cols[p.evolution.bonusJ].nb != VIDE) && (p.cols[p.evolution.bonusJ].couleur == JAU))
	{
		s.nbJ++;
	}
	if ((p.cols[p.evolution.bonusJ].nb != VIDE) && (p.cols[p.evolution.bonusJ].couleur == ROU))
	{
		s.nbR++;
	}
	if ((p.cols[p.evolution.malusR].nb != VIDE) && (p.cols[p.evolution.malusR].couleur == JAU))
	{
		s.nbJ--;
	}
	if ((p.cols[p.evolution.malusR].nb != VIDE) && (p.cols[p.evolution.malusR].couleur == ROU))
	{
		s.nbR--;
	}
	if ((p.cols[p.evolution.malusJ].nb != VIDE) && (p.cols[p.evolution.malusJ].couleur == JAU))
	{
		s.nbJ--;
	}
	if ((p.cols[p.evolution.malusJ].nb != VIDE) && (p.cols[p.evolution.malusJ].couleur == ROU))
	{
		s.nbR--;
	}
