...
#define CHECK_IF(sts, val, msg)                    \
	if ((sts) == (val))                            \
	{                                              \
		fprintf(stderr, "erreur appel systeme\n"); \
		perror(msg);                               \
		exit(-1);                                  \
	}
...
{
	octet nbJ;
	octet nbJ5; // total et piles de 5 pour les jaunes
	octet nbR;
	octet nbR5; // total et piles de 5 pour les rouges
} T_Score;
...
octet estValide(T_Position p, octet origine, octet destination); // ajout 20/03/2022